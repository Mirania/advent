...
export type ProgramState = { base: number, code: number[], limit: number, ptr: number, halted: boolean };
...
export function state(): ProgramState {
    return {base: relativebase, code: intcode, limit: pointerlimit, ptr: i, halted: halted};
}
...
export function cpu(input: number): number {
    while (i < pointerlimit) {     
        switch (intcode[i] % 100) { //last 1 or 2 digits of instruction
            ...
            case 99: halted = true; throw new Error("halted");
        }
    }
}
